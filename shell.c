...
int runCommand(struct job newJob, struct jobSet * jobList,
    ...
    if (!strcmp(newJob.progs[0].argv[0], "exit")) {
        ...
        char *status = NULL;
        for(job = jobList->head; NULL != job; job = job->next) {
          status = (job->runningProgs == job->stoppedProgs) ? STOPPED_STATUS : RUNNING_STATUS;
          printf(JOB_STATUS_FORMAT, job->jobId, status, job->text);
        }
        return 0;
               ...
               !strcmp(newJob.progs[0].argv[0], "bg")) {
                 ...
                 if(NULL == newJob.progs[0].argv[1]) {
                   printf("%s: requires an argument in the form of %%N where N is the job id.", newJob.progs[0].argv[0]);
                   return 1;
                 }

                 // if only % was inputted or it doesn't start with %
                 if (('%' != newJob.progs[0].argv[1][0]) || ('\0' == newJob.progs[0].argv[1][1])) {
               		printf("%s: requires an argument in the form of %%N where N is the job id.",newJob.progs[0].argv[0]);
               		return 1;

               	 }

                 // converts jobID to an integer
                 errno = 0;
                 jobId = strtol(&newJob.progs[0].argv[1][1], NULL, 10);
                 if(0 != errno) {
                    printf("%s: Error converting it to integer.\n", newJob.progs[0].argv[0], &newJob.progs[0].argv[1][1]);
               		return 1;
                 }

                 // search the joblist for the jobId (which is an long now)
                 for(job = jobList->head; NULL != job; job = job->next) {
                   if(jobId == job->jobId) {
                     break;
                   }
                 ...
                 if(NULL == job) {
                   printf("%s: no %ld job id in the list", newJob.progs[0].argv[0], jobId);
                   return 1;
                 }

                 // if the command is already in the foreground
                 if('f' == newJob.progs[0].argv[0][0]) {
                   if(0 != tcsetpgrp(STDIN_FILENO, job->pgrp)) {
                     printf("%s: tcsetpgrp has failed.\n", newJob.progs[0].argv[0]);\
                     perror("");
                     return 1;
                   }

                   // set the given job to be the foreground job
                   jobList->fg = job;
                 }

                 // continue the job
                 if (0 != kill(-job->pgrp, SIGCONT))
               	{
               		printf("%s: Failed to continue job %ld.\n", newJob.progs[0].argv[0], jobId);
               		perror("");
               		return 1;
               	}

                // update isStopped attribute
                job->stoppedProgs = 0;
